...
Python 3.11+ and a GPU with OpenGL 3.3 (there is a CPU fallback, but the
cutaway needs the GPU path).
...
`requirements.txt` pulls VertexUI straight from its repository — it is not
on PyPI, and Bedside needs a build with the `Toasts` `on_event` hook.
...
## Why not the OctoPrint web UI

- Opens in about a second, no browser tab, no re-login
- The 3D view shows exactly how far through the model the printer is —
  driven by real byte offsets, not an estimate from percent complete
- Click-through toasts report a finished print *over* a fullscreen game
- Controls are locked while a print runs and need a deliberate unlock

The rest of this file is the engineering log: what broke, what the
measurements said, and why each fix is the shape it is.
...
First launch asks for the printer address. **Pair with OctoPrint** starts
the Application Keys handshake: OctoPrint shows an authorisation prompt in
its own web UI, you click Allow, and the key arrives here — never typed
anywhere else. There is a manual key box for older OctoPrint builds without
the plugin.

Settings live in `%APPDATA%\bedside\settings.json`, theme presets in
`%APPDATA%\bedside\themes\`, both courtesy of VertexUI's `Settings`.

## Building the exe

```bash
.venv\Scripts\pyinstaller.exe --noconfirm Bedside.spec
```

Output is `dist/Bedside/Bedside.exe` — or just run `Bedside.bat` in the
project root, which points at it.

> PyInstaller also leaves an `Bedside.exe` in **`build/Bedside/`**. That
> one is an intermediate bootloader stub with no `_internal` folder, and
> running it fails with *"Failed to load Python DLL … python314.dll"*.
> `build/` is scratch; the app is always in `dist/`.

It is a **onedir** build on purpose:
a `--onefile` exe has to unpack ~150MB of imgui-bundle, numpy and OpenCV to
a temp directory on every launch, which costs several seconds of startup for
an app you open to glance at a print. Add `--onefile` if you would rather
